...
Blocking::get_edges_of_node(const Node AN)
{
	...
	for (auto it = m_cmap.one_dart_per_incident_cell<1, 0>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<1, 0>(d).end(); it != itend; ++it) {
...
Blocking::get_faces_of_node(const Node AN)
{
	...
	Dart3 d = AN->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<2, 0>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<2, 0>(d).end(); it != itend; ++it) {
...
Blocking::get_edges_of_face(const Face AF)
{
	std::vector<Edge> edges;
	Dart3 d = AF->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<1, 2>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<1, 2>(d).end(); it != itend; ++it) {
		edges.push_back(m_cmap.attribute<1>(it));
	}
	return edges;
}
...
Blocking::get_blocks_of_node(const Node AN)
{
	...
	Dart3 d = AN->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<3, 0>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<3, 0>(d).end(); it != itend; ++it) {
...
Blocking::get_faces_of_edge(const Edge AE)
{
	std::vector<Face> faces;
	Dart3 d = AE->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<2, 1>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<2, 1>(d).end(); it != itend; ++it) {
		faces.push_back(m_cmap.attribute<2>(it));
	}
	return faces;
}
...
Blocking::get_blocks_of_edge(const Edge AE)
{
	std::vector<Block> blocks;
	Dart3 d = AE->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<3, 1>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<3, 1>(d).end(); it != itend; ++it) {
		blocks.push_back(m_cmap.attribute<3>(it));
	}
	return blocks;
}
...
Blocking::get_blocks_of_face(const Face AF)
{
	std::vector<Block> blocks;
	Dart3 d = AF->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<3, 2>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<3, 2>(d).end(); it != itend; ++it) {
		blocks.push_back(m_cmap.attribute<3>(it));
	}
	return blocks;
}
...
Blocking::get_nodes_of_block(const Blocking::Block AB)
{
	...
	nodes[3] = m_cmap.attribute<0>(m_cmap.alpha<1, 0>(d1));

	Dart3 d2 = m_cmap.alpha<2, 1, 0, 1, 2>(d1);
	nodes[4] = m_cmap.attribute<0>(d2);
	nodes[5] = m_cmap.attribute<0>(m_cmap.alpha<0, 1>(d2));
	nodes[6] = m_cmap.attribute<0>(m_cmap.alpha<0, 1, 0, 1>(d2));
	nodes[7] = m_cmap.attribute<0>(m_cmap.alpha<1, 0>(d2));
...
Blocking::get_nodes_of_edge(const Blocking::Edge AE)
{
	...
	auto d0_node = m_cmap.attribute<0>(m_cmap.alpha<0>(d));
                       ...
                       const math::Point &AP8)
{
	...
	m_cmap.set_attribute<0>(m_cmap.alpha<0>(d1), create_node(4, NullID, AP2));
	m_cmap.set_attribute<0>(m_cmap.alpha<0, 1, 0>(d1), create_node(4, NullID, AP3));
	m_cmap.set_attribute<0>(m_cmap.alpha<1, 0>(d1), create_node(4, NullID, AP4));
	m_cmap.set_attribute<0>(m_cmap.alpha<2, 1, 0, 1, 2>(d1), create_node(4, NullID, AP5));
	m_cmap.set_attribute<0>(m_cmap.alpha<2, 1, 0, 1, 2, 0, 1>(d1), create_node(4, NullID, AP6));
	m_cmap.set_attribute<0>(m_cmap.alpha<2, 1, 0, 1, 2, 0, 1, 0, 1>(d1), create_node(4, NullID, AP7));
	m_cmap.set_attribute<0>(m_cmap.alpha<2, 1, 0, 1, 2, 1, 0>(d1), create_node(4, NullID, AP8));

	// go through all the edges
	for (auto it = m_cmap.one_dart_per_incident_cell<1, 3>(d1).begin(), itend = m_cmap.one_dart_per_incident_cell<1, 3>(d1).end(); it != itend; ++it) {
		m_cmap.set_attribute<1>(it, create_edge(4, NullID));
	}

	// go through all the faces
	for (auto it = m_cmap.one_dart_per_incident_cell<2, 3>(d1).begin(), itend = m_cmap.one_dart_per_incident_cell<2, 3>(d1).end(); it != itend; ++it) {
	...
	for (auto it_r = m_cmap.attributes<3>().begin(), it_rend = m_cmap.attributes<3>().end(); it_r != it_rend; ++it_r) {
		...
		for (auto f : cell_faces) {
			...
			if (m_cmap.is_free<3>(d)) {
				...
				for (auto it_f = m_cmap.attributes<2>().begin(), it_fend = m_cmap.attributes<2>().end(); it_f != it_fend; ++it_f) {
					...
					if (d2 != d && m_cmap.is_free<3>(d2)) {
						...
						if (!one_missing) {
							...
							TCellID ref_id_0 = n2n[m_cmap.attribute<0>(m_cmap.alpha<0>(d))->info().topo_id];
							...
							while (n2n[m_cmap.attribute<0>(d_glue)->info().topo_id] != ref_id) {
								d_glue = m_cmap.alpha<1, 0>(d_glue);
							}
							if (n2n[m_cmap.attribute<0>(m_cmap.alpha<0>(d_glue))->info().topo_id] == ref_id_0) {
								m_cmap.sew<3>(d_glue, d);
							}
							else {
								m_cmap.sew<3>(m_cmap.alpha<1>(d_glue), d);
							}
						}
					}
				}
...
Blocking::get_all_sheet_darts(const Edge AE, std::vector<Dart3> &ADarts)
{
	...
	if (m_cmap.attribute<0>(first_dart)->info().topo_id > m_cmap.attribute<0>(m_cmap.alpha<0>(first_dart))->info().topo_id)
		first_dart = m_cmap.alpha<0>(first_dart);
	...
	m_cmap.mark_cell<1>(AE->dart(), edge_mark);
	...
	while (!front.empty()) {
		...
		for (CMap3::Dart_of_orbit_range<2, 3>::iterator it(m_cmap.darts_of_orbit<2, 3>(d).begin()), itend(m_cmap.darts_of_orbit<2, 3>(d).end()); it != itend;
		     ++it) {
			auto d_next_edge = m_cmap.alpha<1, 0, 1>(it);
...
Blocking::get_all_chord_blocks(const Face AF)
{
	...
	for (auto d : face_darts) {
		...
		if (!m_cmap.is_marked(m_cmap.alpha<3>(d), block_mark)) {
			bls.push_back(m_cmap.attribute<3>(m_cmap.alpha<3>(d)));
			m_cmap.mark_cell<3>(m_cmap.alpha<3>(d), block_mark);
		}
	}
...
Blocking::get_all_chord_darts(const Face AF, std::vector<Dart3> &ADarts)
{
	...
	if (!m_cmap.is_free<3>(AF->dart())) front.push_back(m_cmap.alpha<3>(AF->dart()));
	...
	while (!front.empty()) {
		...
		auto d_next_face = m_cmap.alpha<2, 1, 0, 1, 2, 3>(d);
		if (!m_cmap.is_marked(d_next_face, face_mark)) {
			...
			if (!m_cmap.is_free<3>(d_next_face)) front.push_back(m_cmap.alpha<3>(d_next_face));
		}
	}
	// We must unmark all the marked edges. As we stored one dart per edge, it is straightforward
	for (auto d : ADarts) {
...
Blocking::cut_sheet(const Edge AE, const double AParam)
{
	...
	for (auto d : sheet_darts) {
		Dart3 d0 = m_cmap.alpha<0>(d);
		math::Point pa = m_cmap.attribute<0>(d)->info().point;
		math::Point pb = m_cmap.attribute<0>(d0)->info().point;
		auto edge_att = m_cmap.attribute<1>(d)->info();

		Dart3 d_edge = m_cmap.insert_cell_0_in_cell_1(d);     // d_edge = alpha<0,1>(d)
		...
		m_cmap.set_attribute<0>(d_edge, create_node(edge_att.geom_dim, edge_att.geom_id, pc));
	}
	...
	for (auto d : in_edge_darts) {
		if (!m_cmap.is_marked(d, mark_done)) {
			...
			Dart3 d2 = m_cmap.alpha<0, 1, 0, 1, 0>(d);
			if (!m_cmap.is_marked(d2, mark_edge_darts)) {
				d2 = m_cmap.alpha<1, 0>(d2);
			}
			...
			m_cmap.mark(d, mark_done);
			m_cmap.mark(d2, mark_done);
			// the face can be shared by two regions and so, we must mark the right darts
			if (!m_cmap.is_free<3>(d)) {
				m_cmap.mark(m_cmap.alpha<3>(d), mark_done);
				m_cmap.mark(m_cmap.alpha<3>(d2), mark_done);
			}
		}
	}
	...
	for (auto d : in_face_darts) {
		if (!m_cmap.is_marked(d, mark_done)) {
			...
			do {
				...
				m_cmap.mark(m_cmap.alpha<0>(current_dart), mark_done);
				current_dart = m_cmap.alpha<0, 1, 2, 1>(current_dart);
			...
			m_cmap.set_attribute<2>(d_block, create_face(3, NullID));
		}
	}
...
Blocking::mark_half_face(gecko::gblock::Dart3 ADart, int AMark)
{
	...
	do {
		m_cmap.mark(d, AMark);
		m_cmap.mark(m_cmap.alpha<0>(d), AMark);
		d = m_cmap.alpha<0, 1>(d);
	} while (d != ADart);
}
/*----------------------------------------------------------------------------*/
...
Blocking::compute_pillow_twin_node(const Node &ANode, const int AMark)
{
	...
	Dart3 blue_dart;
	for (auto it(m_cmap.darts_of_orbit<1, 2, 3>(d).begin()), itend(m_cmap.darts_of_orbit<1, 2, 3>(d).end()); it != itend && !found_blue_dart; ++it) {
	...
	for (auto it = m_cmap.one_dart_per_incident_cell<2, 0>(blue_dart).begin(), itend = m_cmap.one_dart_per_incident_cell<2, 0>(blue_dart).end(); it != itend;
	     ++it) {

		auto fi = m_cmap.attribute<2>(it);
		if (m_cmap.is_marked(it, AMark) || m_cmap.is_marked(m_cmap.alpha<3>(it), AMark)) {
			nb_incident_blue_faces++;
		}
	}
	auto nb_incident_blue_blocks = 0;
	auto mark_block = m_cmap.get_new_mark();
	for (auto it = m_cmap.one_dart_per_incident_cell<3, 0>(blue_dart).begin(), itend = m_cmap.one_dart_per_incident_cell<3, 0>(blue_dart).end(); it != itend;
	     ++it) {
		...
		for (auto it2 = m_cmap.one_dart_per_incident_cell<2, 0>(it).begin(), it2end = m_cmap.one_dart_per_incident_cell<2, 0>(it).end(); it2 != it2end; ++it2) {
			...
			else if (m_cmap.is_marked(m_cmap.alpha<3>(it2), AMark) && m_cmap.is_marked(m_cmap.alpha<3>(it2), mark_block)) {
				found_blue_dart_in_block = true;
			}
		}
	...
	if ((nb_incident_blue_faces == 1 && nb_incident_blue_blocks == 1) || (nb_incident_blue_faces == 2 && nb_incident_blue_blocks == 2)
		...
		Dart3 edge_dart = m_cmap.alpha<2, 1>(blue_dart);
...
Blocking::pillow(std::vector<Face> &AFaces)
{
	...
	for (auto i_f = 0; i_f < AFaces.size() && !found_seed; i_f++) {
		...
		if (!m_cmap.is_free<3>(di)) {
			...
			mark_half_face(m_cmap.alpha<3>(di), mark_red_side);
		}
	}
	if (!found_seed) {
			...
			for (auto f : AFaces) {
				...
				if (!m_cmap.is_marked(d, mark_blue_side) && !m_cmap.is_marked(d, mark_red_side)) {
					...
					do {
						...
						Dart3 edge_dart = m_cmap.alpha<2>(current_dart);
						...
						do {
							if (m_cmap.is_marked(edge_dart, mark_blue_side)) {
								...
								edge_dart = m_cmap.alpha<3, 2>(edge_dart);
							}
						} while (edge_dart != current_dart && edge_dart != m_cmap.alpha<2>(current_dart) && !found_blue && !found_red);
						// color the face?
						if (found_blue) {
							...
							if (!m_cmap.is_free<3>(current_dart)) {
								mark_half_face(m_cmap.alpha<3>(current_dart), mark_red_side);
							}
						}
						else if (found_red) {
							color_current_face = true;
							...
							if (!m_cmap.is_free<3>(current_dart)) {
								mark_half_face(m_cmap.alpha<3>(current_dart), mark_blue_side);
							}
						}
						// we go to the next dart
						current_dart = m_cmap.alpha<0, 1>(current_dart);
		...
		for (auto f : AFaces) {
			...
			else if ((m_cmap.is_marked(d, mark_blue_side) && m_cmap.is_marked(m_cmap.alpha<3>(d), mark_red_side))
			         || (m_cmap.is_marked(d, mark_red_side) && m_cmap.is_marked(m_cmap.alpha<3>(d), mark_blue_side))) {
				nb_blue_and_red++;
			}
			else
				throw GMDSException("Error during blue-red coloring of face");
	...
	for (auto e : surf_edges) {
		...
		auto [n1_tdim, n1_tid, n1_gdim, n1_gid] = pillow_node_data[m_cmap.alpha<0>(d_edge)];
	...
	for (auto f : AFaces) {
		...
		if (m_cmap.is_marked(df, mark_blue_side))
			...
			blue_darts.push_back(m_cmap.alpha<3>(df));
	}
	...
	for (auto d : blue_darts) {
		if (m_cmap.is_free<3>(d)) {
			...
			was_sewed[d] = true;
			prev_3_dart[d] = m_cmap.alpha<3>(d);
			m_cmap.unsew<3>(d);
		}
	}
	...
	for (auto df : blue_darts) {
		...
		Dart3 db_min = db;
		Dart3 db_cur = m_cmap.alpha<0, 1>(db);
		while (db_cur != db) {
			...
			db_cur = m_cmap.alpha<0, 1>(db_cur);
		}
		// We must now to check if df must be sewed to db_min or a<1>(db_min)
		math::Point pnt_df0 = m_cmap.attribute<0>(m_cmap.alpha<0>(df))->info().point;
		math::Point pnt_db_min0 = m_cmap.attribute<0>(m_cmap.alpha<0>(db_min))->info().point;
		math::Point pnt_db_min10 = m_cmap.attribute<0>(m_cmap.alpha<1, 0>(db_min))->info().point;
		if (pnt_db_min10.distance2(pnt_df0) < pnt_db_min0.distance2(pnt_df0)) {
			// switch the edge s
			db_min = m_cmap.alpha<1>(db_min);
		}

		// we copy the face attribute
		Dart3 d_opp_face = m_cmap.alpha<2, 1, 0, 1, 2>(db_min);
		if (was_sewed[df]) {
		...
		do {
			...
			if (!m_cmap.is_marked(d, mark_already_merged_node)) {
				...
				Dart3 d_opp = m_cmap.alpha<3, 2, 1, 0, 1, 2>(d);
				...
				Dart3 d_edge = m_cmap.alpha<3, 2, 1>(d);
			...
			if (m_cmap.is_marked(d, mark_already_merged_edge)) {
				...
				do {
					if (m_cmap.is_marked(d3, mark_blue_side)) {
						...
						d3 = m_cmap.alpha<3, 2>(d3);
					}
				} while (!found);
				m_cmap.sew<3>(m_cmap.alpha<3, 2>(d), m_cmap.alpha<3, 2>(d3));
			}
			else {
				...
				Dart3 d_face = m_cmap.alpha<3, 2>(d);
...
Blocking::collapse_chord(const Face AF, const Node AN1, const Node AN2)
{
	...
	for (CMap3::Dart_of_orbit_range<0, 1, 3>::iterator it(m_cmap.darts_of_orbit<0, 1, 3>(dart_f).begin()), itend(m_cmap.darts_of_orbit<0, 1, 3>(dart_f).end());
	     it != itend; ++it) {
	...
	for (auto d2 : all_darts_n2) {
		for (auto d1 : all_darts_n1) {
			if (m_cmap.alpha<0, 1, 0>(d1) == d2) n1_is_n2_opposite = true;
		}
	}
	...
	for (auto d : chord_darts) {
		chord_darts_opp.push_back(m_cmap.alpha<0, 1, 0>(d));
	}
	...
	for (auto i = 0; i < chord_darts.size(); i++) {
		...
		to_sew.push_back(std::make_pair(m_cmap.alpha<2, 3>(di), m_cmap.alpha<2, 3>(dj)));
		to_sew.push_back(std::make_pair(m_cmap.alpha<1, 2, 3>(di), m_cmap.alpha<1, 2, 3>(dj)));
		if (!m_cmap.is_free<3>(di)) {
			to_sew.push_back(std::make_pair(m_cmap.alpha<3, 2, 3>(di), m_cmap.alpha<3, 2, 3>(dj)));
			to_sew.push_back(std::make_pair(m_cmap.alpha<3, 1, 2, 3>(di), m_cmap.alpha<3, 1, 2, 3>(dj)));
		}
	}
