...
class  Blocking
{
 public:
	...
	{
		if (TDim==0) {
			return m_structure.getNbNodes();
		}
		if (TDim==1) {
			return m_structure.getNbEdges();
		}
		if (TDim==2) {
			return m_structure.getNbFaces();
		}
		if (TDim==3) {
			return m_structure.getNbRegions();
		}
		throw GMDSException("Get number of cells error: inacceptable dimension!");
	}
	 ...
	 */
	std::vector<Region> get_all_blocks();
	 ...
	 */
	Edge get_edge(const int AN1, const int AN2);
	 ...
	 * @return the edge connecting @p AN1 and @p AN2
	 */
	Edge get_edge(const Node AN1, const Node AN2);
	 ...
	 */
	std::vector<Region> get_blocks_of_node(const Node AN);
	 ...
	 */
	std::vector<Region> get_blocks_of_edge(const Edge AE);
	 ...
	 */
	std::vector<Region> get_blocks_of_face(const Face AF);
	 ...
	 */
	std::vector<Face> get_faces_of_block(const Region AB);
	/** Get all the edges of a block. If it is a hexahedral block,
	 * we have 12 faces.
	 * @param[in] AB a block
	 * @return the set of edges of the block.
	 */
	std::vector<Edge> get_edges_of_block(const Region AB);
	 ...
	 */
	std::vector<Node> get_nodes_of_block(const Region AB);
	 ...
	 */
	math::Point get_center_of_block(const Region AB);
	 ...
	 */
	std::vector<Region> get_all_chord_blocks(const Face AF);
	 ...
	 */
	std::tuple<Edge, double, double> get_cut_info(const int APointId,
	 ...
	 * and the distance to the edge (third)
	 */
	std::tuple<Edge, double, double> get_cut_info(const gmds::math::Point &APoint,
	                                                        const std::vector<Edge> AEdges);
	 ...
	 */
	Region create_block(const math::Point &AP1,
 ...
 private:
	 ...
	 */
	std::vector<std::pair<double, double>> get_projection_info(const math::Point &AP, std::vector<Edge> &AEdges);
	 ...
	 */
	std::pair<double, double> get_projection_info(const math::Point &AP, Edge &AEdge);
 ...
 private:
	...
	Variable<CellInfo>* m_node_info;
	Variable<CellInfo>* m_edge_info;
	Variable<CellInfo>* m_face_info;
	Variable<CellInfo>* m_block_info;
	/*** id counter*/
	Counter m_counter;
};
/*----------------------------------------------------------------------------*/
