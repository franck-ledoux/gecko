...
`app/`, `python/src/` and `biy/src/` — the whole first-party C++ tree.
...
library modules' `inc/` directories, `app/inc/` included — `python/src/` and `biy/src/` are internal glue, not public
